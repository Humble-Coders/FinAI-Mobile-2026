...
- **A number that already has an account links instead of duplicating.** The app says so and offers to sign in
  to that account; once signed in, the empty account the new method created is removed and the method
  (Google or Apple) is linked, so either now opens the same account. A **verified email** that matches an
  existing account links automatically, without the phone step. Adding a sign-in method notifies the account.
...
| 2026-09-15 | **Ways in: email and password, Google, Apple. Phone OTP is no longer a sign-in route**; every account still verifies a phone once, straight after signup. Email signup is confirmed by emailed code; forgot password resets by emailed code. Name is not captured at signup. **A phone collision offers linking**: sign in to the account that has the number, the empty account the new method created is deleted (database rows and Supabase auth user, only if it has no phone and no financial data), and the method is linked. **A verified email links automatically, including to accounts that already verified a phone**; different verified phones never link. Adding a method notifies the account. Supersedes the 2026-08-29 routes, and the rule that email only links accounts without a phone | Sign-in by SMS alone ties the account to a number that can be lost, ported or recycled, and makes SMS cost and deliverability a login dependency. Once email and password is a way in, whoever controls the address can already reset the password, so refusing to link Google by that same verified address guards a door that is open anyway; the defence against a reassigned address moves to the "method added" notification. The phone stays as the one key that survives Apple Hide My Email (manager decision, 2026-09-15) |
