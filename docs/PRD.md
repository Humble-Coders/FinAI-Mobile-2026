...
- **A number that already has an account links instead of duplicating.** The app says so and offers to sign in
  ...
  existing account links automatically, without the phone step. Adding a sign-in method is recorded in the backend only; the person is not notified.
