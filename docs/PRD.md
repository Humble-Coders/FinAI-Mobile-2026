...
**Every signup verifies a phone immediately after (locked; routes revised 2026-09-15).** Signup is email and password, Google or Apple; each authenticates before any phone number exists. Rather than inferring a region from device signals, the user is asked for their phone number — with country code, verified by OTP — as the **next step after the provider handshake**. Region is then derived from it. There is **one region mechanism, not two.**

**Why required rather than lazy.** The deciding factor is not region, it is **identity**. Apple's *Hide My Email* returns a relay address that matches nothing else the person has used, so a user who signs up with Google and later taps "Continue with Apple" would create a **second household with split financial history** — undetectable at signup and painful to repair. **A verified phone number is the canonical identity key**: email, Google and Apple signups for the same person all converge on it, so the collision is caught at signup instead of discovered months later. In a product holding financial records, that outweighs the signup friction it costs. A **verified email** is a second, weaker key (2026-09-15): once email and password is a way in, control of the address already grants the account through a password reset, so matching it adds no new exposure — but only an address the provider marks verified counts, and two different verified phones never link.
...
#### F1. Onboarding
- **Three ways in:** **email and password** (primary), **Google**, and **Apple** — each both a signup and a
  sign-in route (manager decision, 2026-09-15 — §9). Email signup is confirmed with a 6-digit code sent to the
  address; there is a prominent **Forgot password** that resets by emailed code. Sign in with Apple is
  **mandatory on iOS** whenever another third-party provider is offered (App Store guideline 4.8) — a platform
  requirement, not a product choice. Apple is offered on iOS only. The phone number is **not** a sign-in method.
- **Apple returns email and name only on the first authorization**; every later sign-in omits them. The name
  is **not captured at signup** (2026-09-15); a display name is asked for later, where it is needed. Hide My
  Email relay addresses are valid and deliverable — never treat one as fake.
- **Every new account verifies a phone number once, straight after signup**, with country code and SMS code
  (§4.6). Region derives from it, and the verified number is the key that stops one person becoming two
  accounts. Flow: create account (or tap Google/Apple) → phone → 6-digit code. Signing in later never asks for it.
- **A number that already has an account is refused.** The phone step says the number is already registered
  and asks for another; it never offers to sign in to or link with that account (manager decision, 2026-09-15).
  A **verified email** that matches an existing account links automatically, without the phone step. Adding a sign-in method is recorded in the backend only; the person is not notified.
...
**One narrow exception, opt-in and time-boxed (2026-09-21).** When an import **fails or flags most of its rows**, the user may explicitly choose to send that import's **redacted text** — never the document — so the parser can be fixed. It is kept for **at most 30 days** — expiry runs on the import path rather than on a timer, so text expires when the next import happens; at current volumes that can mean a short overhang, and a scheduled sweep is a before-launch item. It is asked per import, never pre-ticked, never on a successful import, and consented separately from the first-import consent: "process my statement" and "keep a copy of its contents for a month" are different questions. It is exported with the user's data and deleted with their account. This is the only thing we retain from an import beyond the transactions themselves, and the reason it is worth it is that the review queue tells us *that* a parse failed while only the text tells us *why*.
...
| 2026-08-29 | ~~Signup routes: phone OTP, Google, and Apple~~ — **superseded 2026-09-15** (email and password replaces phone OTP as a route). All routes still end with a verified phone number; Sign in with Apple is mandatory on iOS under App Store guideline 4.8 | Social signin lowers onboarding friction; Apple is a platform requirement once Google is offered |
...
| 2026-08-29 | **Identity collision prevented by the verified phone number**, with matching-email linking as a secondary path; never silently create a second account (email linking widened 2026-09-15) | A verified phone converges Google, Apple and OTP signups for the same person; email alone cannot, because Apple relay addresses match nothing |
...
4. **Retention enforced in code** — ~~F2's delete-on-confirmation-or-72-hours rule, as a scheduled job~~ **withdrawn 2026-09-21: statements are never received** (F2). What remains is narrower and still enforced in code: **opt-in diagnostic text expires after 30 days**, purged by the parse path itself rather than by a manual process. Stated precisely, because the difference is what a regulator asks about: the purge runs on every import request, so expired text survives until the next import anywhere in the system. At current volumes that gap can exceed a day, so a scheduled sweep is required before launch (`docs/ROADMAP.md`). Retention duties apply in full to transactions, corrections, embeddings and chat history — covered by items 2 and 3.
