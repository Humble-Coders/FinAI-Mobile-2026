...
4. **Retention enforced in code** — ~~F2's delete-on-confirmation-or-72-hours rule, as a scheduled job~~ **withdrawn 2026-09-21: statements are never received** (F2). What remains is narrower and still enforced in code: **opt-in diagnostic text expires after 30 days** and is purged by the parse path itself, not by a manual process or a cron job nobody watches. Retention duties apply in full to transactions, corrections, embeddings and chat history — covered by items 2 and 3.
